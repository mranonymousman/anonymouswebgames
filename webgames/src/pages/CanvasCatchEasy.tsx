...
export const PASSWORD_CanvasCatchEasy = "EasyCircleMaster2024";
export const TASK_ID_CanvasCatchEasy = "canvas-catch-easy";

const CanvasCatchEasy = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_CanvasCatchEasy);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [completed, setCompleted] = useState(false);
  const [circle, setCircle] = useState<Circle>({ x: 100, y: 100, radius: 30 }); // Larger circle
  const targetZone: TargetZone = { x: 350, y: 150, width: 150, height: 150 }; // Larger target zone
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <h1 className="text-2xl font-bold mb-4">Canvas Catch (Easy)</h1>
      {completed ? (
        <div className="mt-6 p-8 bg-green-100 rounded-lg border border-green-200 text-center">
          ...
          <p className="text-green-800">
            You've successfully completed the easy challenge!
          </p>
          <p className="text-2xl font-bold text-green-900 mt-4">
            Secret Password: {PASSWORD_CanvasCatchEasy}
          </p>
        </div>
      ) : (
        <>
          <p className="mb-4">
            Drag the blue circle into the orange box. This one is a bit easier!
          </p>
          <canvas
            ref={canvasRef}
...
export default CanvasCatchEasy;
