...
export const TASK_ID_HerdingHard = "herding-hard";
export const PASSWORD_HerdingHard = "HerdTooHard";

const HerdingHard = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_HerdingHard);
  ...
  useEffect(() => {
    const initialSheep: Sheep[] = Array.from({ length: 10 }, (_, i) => ({
      // Hard: 10 sheep
      ...
      {hasWon ? (
        <div className="text-center">
          ...
          <div className="text-xl bg-yellow-100 p-4 rounded-lg border-2 border-yellow-400">
            ...
            <span className="font-bold">{PASSWORD_HerdingHard}</span>
          </div>
        </div>
      ) : (
