...
export const TASK_ID_ClickCubedEasy = "click-cubed-easy";
export const PASSWORD_ClickCubedEasy = "TripleClickChampionEasy";

const ClickCubedEasy = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_ClickCubedEasy);
  const [gameStarted, setGameStarted] = useState(false);
  const [button1Clicked, setButton1Clicked] = useState(false);
  const [button2Clicked, setButton2Clicked] = useState(false);
  const [timeLeft, setTimeLeft] = useState(10); // Increased time limit
  ...
  const resetGame = () => {
    ...
    setTimeLeft(10); // Reset to increased time limit
    setGameWon(false);
    setGameLost(false);
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <h1 className="text-4xl font-bold mb-8">Click³ Challenge (Easy)</h1>

      <div className="text-xl mb-8">
        {!gameStarted
          ? "Click the first button to start! You'll have 10 seconds to click all three buttons."
