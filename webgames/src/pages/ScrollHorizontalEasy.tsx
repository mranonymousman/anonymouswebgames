...
export const PASSWORD_ScrollHorizontalEasy = "EASYSCROLL2023";
export const TASK_ID_ScrollHorizontalEasy = "scroll-horizontal-easy";

const ScrollHorizontalEasy = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_ScrollHorizontalEasy);
  ...
  const numBoxes = 10; // Easy variant: 10 boxes
  ...
  const boxes = Array(numBoxes)
    ...
    .map((_, index) => (
      <div
        ...
        style={{
          backgroundColor: `hsl(${(index * 25) % 360}, 70%, 80%)`,
  ...
  return (
    <div>
      <div className="fixed top-0 left-0 right-0 bg-white p-5 z-10 border-b border-gray-200">
        <h1>Scroll to the Right (Easy)</h1>
      ...
      <div className="mt-[100px] whitespace-nowrap overflow-x-auto p-5">
        ...
        {isLastBoxVisible && (
          ...
          >
            You made it! Password: {PASSWORD_ScrollHorizontalEasy}
          </div>
        )}
      </div>
    </div>
...
export default ScrollHorizontalEasy;
