...
export const PASSWORD_MapPannerEasy = "CARTOGRAPHEREASY2024";
export const TASK_ID_MapPannerEasy = "map-panner-easy";
...
const MapPannerEasy: React.FC = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_MapPannerEasy);
  ...
  const treasureLocation = { x: 500, y: 400 }; // Closer for easy mode
  const treasureRadius = 200; // Larger radius for easy mode
  ...
  return (
    <div className="w-full h-screen overflow-hidden bg-gray-900 text-white">
      <div className="fixed top-4 left-4 z-10 bg-gray-800 p-4 rounded-lg shadow-lg">
        <h1 className="text-2xl font-bold mb-2">Map Panner (Easy)</h1>
        <p className="text-gray-300">
          {isComplete
            ? "You found the treasure!"
            : "Pan around the map to find the hidden treasure. Try searching in the northeast quadrant of the map, around coordinates (400-600, 300-500)..."}
        </p>
        {isComplete && (
          <div className="mt-4 p-4 bg-green-800 rounded-lg">
            <p className="font-bold">Password: {PASSWORD_MapPannerEasy}</p>
          </div>
        )}
      </div>
