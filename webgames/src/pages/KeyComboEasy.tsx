...
export const PASSWORD_KeyComboEasy = "EASY_KEY_MASTER_2024";
export const TASK_ID_KeyComboEasy = "key-combo-easy";

const KeyComboEasy = () => {
  const { recordSuccess } = useTaskAnalytics(TASK_ID_KeyComboEasy);
  const [pressedKeys, setPressedKeys] = useState<Set<string>>(new Set());
  const [isComplete, setIsComplete] = useState(false);

  // Define the target key combination
  const comboDisplay = "Shift + A";

  useEffect(() => {
    const targetCombo = ["Shift", "a"];
      ...
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-6">
        ...
        {isComplete && (
          <div className="text-center p-4 bg-green-100 text-green-700 rounded-lg">
            ...
            <p>The secret password is: {PASSWORD_KeyComboEasy}</p>
          </div>
        )}
      </div>
    </div>
